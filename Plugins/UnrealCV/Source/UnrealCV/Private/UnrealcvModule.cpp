...
void FUnrealCVPlugin::StartupModule()
{
	...
	{
		UE_LOG(LogUnrealCV, Warning, TEXT("Invalid configured port %d, resetting to 9000"), Server.Config.Port);
		Server.Config.Port = 9000;
	}

	int OverridePort = Server.Config.Port;
	const bool bHasUnrealCVPortOverride =
		FParse::Value(FCommandLine::Get(), TEXT("UnrealCVPort="), OverridePort) ||
		FParse::Value(FCommandLine::Get(), TEXT("cvport="), OverridePort);
	if (bHasUnrealCVPortOverride) {
		...
		{
			UE_LOG(LogUnrealCV, Warning, TEXT("Ignoring invalid UnrealCVPort override: %d"), OverridePort);
		}
	}
